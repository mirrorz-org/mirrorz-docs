镜像站提供了 Julia 的官方包注册表 [General](https://github.com/JuliaRegistries/General) 镜像来加速 Julia 包的安装。

一些镜像站也提供了 Julia 二进制程序的镜像，关于其使用请参考 [Julia Releases 帮助](../julia-releases/).
...
* Linux Bash
  <tmpl z-lang="bash">
  export JULIA_PKG_SERVER={{endpoint}}
</tmpl>

* Windows Powershell:
  <tmpl z-lang="powershell">
  $env:JULIA_PKG_SERVER = '{{endpoint}}'
</tmpl>
...
<tmpl z-lang="bash" z-path="~/.bashrc" z-append>
export JULIA_PKG_SERVER={{endpoint}}
</tmpl>
...
<tmpl z-lang="julia" z-path="~/.julia/config/startup.jl" z-append>
ENV["JULIA_PKG_SERVER"] = "{{endpoint}}"
</tmpl>
