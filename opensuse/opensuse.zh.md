...
5. `软件源名称`（Repository Name）和 `URL` 填写为上文 Leap 或 Tumbleweed 中的仓库和对应连接，如：
    ...
    - `URL`：{ztmpl}`{{endpoint}}/distribution/leap/15.5/repo/oss/`
    ...
    |发行版 | 仓库名称|URL|备注|
    |:-:|:-:|:-:|:-:|
    |Leap|OSS|{ztmpl}`{{endpoint}}/distribution/leap/$releasever/repo/oss/`|-|
    |Leap|NON-OSS|{ztmpl}`{{endpoint}}/distribution/leap/$releasever/repo/non-oss/`|-|
    |Leap|UPDATE|{ztmpl}`{{endpoint}}/update/leap/$releasever/oss/`|-|
    |Leap|UPDATE-NON-OSS|{ztmpl}`{{endpoint}}/update/leap/$releasever/non-oss/`|-|
    |Leap|SLE-UPDATE|{ztmpl}`{{endpoint}}/update/leap/$releasever/sle/`|仅使用 Backport 时需要添加|
    |Leap|SLE-BACKPORT-UPDATE|{ztmpl}`{{endpoint}}/update/leap/$releasever/backports/`|仅使用 Backport 时需要添加|
    |Tumbleweed|OSS|{ztmpl}`{{endpoint}}/tumbleweed/repo/oss/`|-|
    |Tumbleweed|NON-OSS|{ztmpl}`{{endpoint}}/tumbleweed/repo/non-oss/`|-|
    |Tumbleweed|UPDATE|{ztmpl}`{{endpoint}}/update/tumbleweed/`|-|
    |Slowroll|OSS|{ztmpl}`{{endpoint}}/slowroll/repo/oss/`|-|
    |Slowroll|NON-OSS|{ztmpl}`{{endpoint}}/slowroll/repo/non-oss/`|-|
    |Slowroll|UPDATE|{ztmpl}`{{endpoint}}/update/slowroll/repo/oss/`|-|
