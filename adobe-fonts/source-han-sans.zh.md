...
思源黑体是一款泛中日韩无衬线黑体，支持多种字重，于2014年首次发布，近年仍在积极维护。
...
1. **字重范围**
   ...
   可变（variable）版本更灵活，文件总体积也更小（例如 OTF SC 版覆盖七种字重，可变版本只需单个 ~50 MB 文件，而静态版本需要七个 ~20 MB 文件）；但它依赖 2016 年发布的 OpenType 可变字体技术，某些平台尚不兼容（例如 [Typst 导出 SVG 时目前只能加载最细字重](https://github.com/typst/typst/issues/185)）。
...
2. **字形覆盖范围**
   ...
   如果您只用单一语言，不需要其它地区的字符及字符，那推荐选 SubsetOTF（选其它亦可，只不过文件更大）。对于中国大陆用户，一般不需要「가ᇰ」「𰻞」字符和「氺」字形的「雨」，那么选择 SubsetOTF 的 CN 版即可。
