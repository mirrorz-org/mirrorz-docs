...
从OpenWrt 25.12开始，包管理器由 `opkg` 改为 `apk` (Alpine Package Keeper) ，默认软件源的配置文件路径由 `/etc/opkg/distfeeds.conf` 改为 `/etc/apk/repositories.d/distfeeds.list` 。

### 自动替换

对于OpenWrt 25.12及以上版本，需选择'APK'；对于OpenWrt 24.10及以下版本，需选择'OPKG'。
